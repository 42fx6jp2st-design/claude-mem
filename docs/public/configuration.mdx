...
| `CLAUDE_MEM_API_TOKEN`        | —                               | Environment variable for API auth on mutating routes when host is non-loopback |
...
When `CLAUDE_MEM_WORKER_HOST` is configured to a non-loopback address, non-local mutating `/api/*` requests require `CLAUDE_MEM_API_TOKEN` via either the `Authorization: Bearer <token>` header or the `x-claude-mem-token` header.
